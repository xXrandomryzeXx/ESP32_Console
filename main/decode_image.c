...
{
    //*pixels = NULL;
    //uint8_t pixels_size = sizeof(pixels);
    esp_err_t ret = ESP_OK;
    ...
    /*if(size == 0){
        //free(*pixels);
        printf("Trying to allocate %d memory from available, %d memory\n",
                IMAGE_H * IMAGE_W * sizeof(uint16_t),
    ...
    *pixels = heap_caps_malloc(pixels_size, MALLOC_CAP_DEFAULT);
    ...
    printf("Memory of image_start: %p\n", image_start);

    // JPEG decode
    esp_jpeg_image_output_t outimg;
    esp_jpeg_decode(&jpeg_cfg, &outimg);
    printf("JPEG DECODED\n");
    //esp_jpeg_decode(&jpeg_cfg, NULL);
